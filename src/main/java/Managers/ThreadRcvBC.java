...
public class ThreadRcvBC extends Thread {
    ...
    public void run() {
        try {
            ...
            DatagramSocket socket = new DatagramSocket(Self.portUDP);
            ...
            while (true) { //the thread will be in a receiving state constantly
                ...
                } else if (!(rcvNotif.getAddress().getHostName().contains(Self.getInstance().getHostname()))){ //on ne reçoit pas de nous mêmes
